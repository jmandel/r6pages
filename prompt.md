...
Use this prompt to review one FHIR specification HTML source page at a time. Replace every `{{PLACEHOLDER}}` before running it.

## Inputs

- Page path: `{{PAGE_PATH}}`
- Page label: `{{PAGE_LABEL}}`
- Older version: `{{OLD_VERSION_LABEL}}`
- Older source commit: `{{OLD_COMMIT}}`
- Older source tree: `{{OLD_SOURCE_TREE}}`
- Newer version: `{{NEW_VERSION_LABEL}}`
- Newer source commit: `{{NEW_COMMIT}}`
- Newer source tree: `{{NEW_SOURCE_TREE}}`
- Known Jira issue keys: `{{KNOWN_ISSUE_KEYS}}`
- Page topic hints: `{{PAGE_TOPIC_HINTS}}`
- Detailed research output path: `{{RESEARCH_OUTPUT_PATH}}` (batch convention: `todo/research/{{PAGE_LABEL}}.md`)
- Actionable findings output path: `{{ACTION_OUTPUT_PATH}}` (batch convention: `todo/actions/{{PAGE_LABEL}}.md`)

## Role and goal

You are reviewing how one main FHIR specification page changed between `{{OLD_VERSION_LABEL}}` and `{{NEW_VERSION_LABEL}}`.
...
1. A detailed research appendix in `{{RESEARCH_OUTPUT_PATH}}`.
2. A concise, high-signal actionable findings file in `{{ACTION_OUTPUT_PATH}}`.
...
Confirm whether `{{PAGE_PATH}}` exists in both trees:

```bash
test -f "{{OLD_SOURCE_TREE}}/{{PAGE_PATH}}" && echo "old exists"
test -f "{{NEW_SOURCE_TREE}}/{{PAGE_PATH}}" && echo "new exists"
```
...
Use a rename-aware diff from `{{OLD_COMMIT}}` to `{{NEW_COMMIT}}`:

```bash
git -C "{{NEW_SOURCE_TREE}}" --no-pager diff --find-renames \
  "{{OLD_COMMIT}}" "{{NEW_COMMIT}}" -- "{{PAGE_PATH}}"
```
...
```bash
git -C "{{NEW_SOURCE_TREE}}" --no-pager log --reverse --date=short \
  --format='%H%x09%ad%x09%s' "{{OLD_COMMIT}}..{{NEW_COMMIT}}" -- "{{PAGE_PATH}}"
```

Inspect likely meaningful commits with:

```bash
git -C "{{NEW_SOURCE_TREE}}" --no-pager show --stat --patch <COMMIT> -- "{{PAGE_PATH}}"
```
...
Start with every explicit Jira key found in commits, patches, source comments, `{{KNOWN_ISSUE_KEYS}}`, or related PR text:
...
After the diff/history/community investigation, read the full `{{NEW_VERSION_LABEL}}` version of the page:

```bash
wc -l "{{NEW_SOURCE_TREE}}/{{PAGE_PATH}}"
sed -n '1,220p' "{{NEW_SOURCE_TREE}}/{{PAGE_PATH}}"
sed -n '221,440p' "{{NEW_SOURCE_TREE}}/{{PAGE_PATH}}"
```
...
- cited commits actually change `{{PAGE_PATH}}`
...
Write `{{RESEARCH_OUTPUT_PATH}}`. This file may be detailed and is allowed to preserve evidence that is not directly actionable. Keep it organized for later audit, but do not try to make every contextual hit sound important.

Make this appendix a fast handoff for a future reviewer. Include direct links or precise locators for evidence:

- local source file paths and line ranges, such as `{{NEW_SOURCE_TREE}}/{{PAGE_PATH}}:123`
...
Write `{{ACTION_OUTPUT_PATH}}`. This file must be high signal and much shorter than the research appendix.
...
```markdown
# {{PAGE_LABEL}} actionable findings
